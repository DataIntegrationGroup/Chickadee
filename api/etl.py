...
def transform(data):
    for row in data:
        ...
        for k, ki, cast in (
            ...
            ('geologic_unit', "Geologic Unit", str),
            ('map_unit', "Map Unit", str),
            ('region', "Region", str),
            ('monitor_age', "Monitor Age", float),
        ):
            v = row[ki].strip()
            try:
                row[k] = cast(v)
...
def make_sample_payload(row):
    ...
    return {
        ...
        "properties": {
            ...
            "monitor_age": {"value": row["monitor_age"] or 0, "units": "Ma"},
            "map_unit": {"value": row["map_unit"] or ""},
            "geologic_unit": {"value": row["geologic_unit"] or ""},
            "region": {"value": row["region"] or ""},
            "age": {
                "value": row["age"] or 0,
